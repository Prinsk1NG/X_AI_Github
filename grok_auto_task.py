...
Phase 2 - Differential collection + report (global deadline 65 min):
...
v3.3 changelog:
  - LLM priority swapped: Claude first, Kimi fallback
  - PHASE1_DEADLINE 20min -> 30min; GLOBAL_DEADLINE 45min -> 65min
  - S-tier threshold: max_l>3000 & days<=7 (was 10000 & 7)
  - A-tier threshold: max_l>800  & days<=14 (was 1000 & 14)
  - enable_grok4_beta: broader button selector, graceful skip
  - Feishu card: ## -> bold sections, remove --- HR artifacts
  - WeChat HTML: ## -> <h3>, --- -> <hr>
"""
...
def enable_grok4_beta(page):
    ...
    # Try multiple known button labels across Grok UI versions
    selectors = [
...
def classify_accounts(meta_results: dict) -> dict:
    ...
    for account, meta in meta_results.items():
        ...
        if days_since > 30:
            classification[account] = "inactive"
        elif max_l > 3000 and days_since <= 7:   # v3.3: was 10000
            classification[account] = "S"
        elif max_l > 800 and days_since <= 14:   # v3.3: was 1000
...
<hr style="border-top: 3px solid #555;">   <!-- 大板块分隔符 -->
...
（根据实际主题数量继续重复上述格式，每个主题之间使用细分割线 `---`，不需要额外空行）
...
- **格式纪律（严格遵守）：**
  ...
  - 每个 ## 段落之间不加多余空行
  - 大板块之间使用 `<hr style="border-top: 3px solid #555;">`（或等效的厚分割线）作为明显的视觉隔断；小主题之间使用 `---` 作为细分割线，放在每个 **🔁 主题标题：副标题** 的前一行。
...
# ==============================================================================
# Claude (OpenRouter) — v3.3: PRIMARY
# ==============================================================================
...
# ==============================================================================
# Kimi (Moonshot API) — v3.3: FALLBACK
# ==============================================================================
...
def _preprocess_md(content_md: str) -> str:
    """
    Convert LLM Markdown to Feishu-compatible format.
    Feishu markdown element does NOT support ## headers or --- HR lines.
    - ### -> **bold**
    ...
    # ## -> bold section header with visual marker
    content_md = re.sub(
        r'^##\s+(.+)$',
        r'\n**▌ \1**',
        content_md,
...
def _split_to_elements(content_md: str) -> list:
    """
    Split preprocessed content into Feishu card elements.
    After _preprocess_md, sections start with \n**▌ ...**.
    Split there so each section is its own element (avoids 4000-char limit).
    Long sections are further split by paragraph.
    """
    # Split at each bold section header
    sections = re.split(r'\n(?=\n\*\*▌)', content_md)
    elements = []
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        # ======================================================================
        # Phase 1: Tiered scan (4 batches × 25, deadline 30 min)
        # ======================================================================
        print("\n" + "=" * 50, flush=True)
    ...
    # ==========================================================================
    # LLM: Claude first (v3.3), Kimi fallback
    # ==========================================================================
    report_text   = ""
    cover_title   = ""
    cover_prompt  = ""
