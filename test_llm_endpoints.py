...
A small diagnostic tool to run from the repository root (Prinsk1NG/X_AI_Github)
to exercise the Kimi (Moonshot) and Claude (OpenRouter) endpoints and capture
detailed request/response information for debugging.

What it captures:
- Request headers (Authorization redacted) and payload size (chars + est tokens)
- HTTP status code, response headers
- Response body (safe utf-8 decode with replacement), and JSON parse attempts
- Full response saved to debug_outputs/<service>_YYYYmmdd_HHMMSS.json
- Exceptions and stack traces

Usage:
  - Put this file in the repo root and run:
      python3 test_llm_endpoints.py --help
  - Typical quick run (will use small built-in prompt unless you pass --prompt-file):
      python3 test_llm_endpoints.py
  - To test with real payload from your data folder:
      python3 test_llm_endpoints.py --prompt-file data/2026-03-10.jsonl
"""
...
def send_post_with_capture(name: str, url: str, headers: Dict[str, Any], json_payload: Dict[str, Any], timeout: int = 300) -> Dict[str, Any]:
    ...
    Sends POST and returns a dict with:
      ...
      - request_payload_chars / est_tokens
      - response_headers
      - response_status
      - response_text (safe decoded, truncated)
      - response_json (if parsed)
      - exception (if any, full traceback)
...
def main():
    ...
    print("\nNext steps:")
    print(" - Inspect the JSON dumps in debug_outputs/ (they include resp.body and status).")
    print(" - If Kimi returns 400: paste the Kimi dump response_text_full or response_json here (remove any Authorization values).")
    print(" - If Claude raises 'latin-1' error locally, confirm the X-Title header is ASCII (we set a default).")
    print(" - When pasting logs here, do NOT include API keys.")
    print("\nDone.")
